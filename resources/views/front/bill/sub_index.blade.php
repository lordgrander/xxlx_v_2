...
<hr>
                        ...
                        <div>
                            @php($display_text = '')
                            @if($buy_orderxo->name=='ABC')
                            @elseif($buy_orderxo->name=='WING')
                                @php($display_text = 'ວິ້ງ') 
                            @elseif($buy_orderxo->name=='TWO')
                                @php($display_text = 'ເລກສອງໂຕ') 
                            @elseif($buy_orderxo->name=='THREE')
                                @php($display_text = 'ເລກສາມໂຕ') 
                            @elseif($buy_orderxo->name=='FOUR')
                                @php($display_text = 'ເລກສີ່ໂຕ') 
                            @elseif($buy_orderxo->name=='FIVE')
                                @php($display_text = 'ເລກຫ້າໂຕ') 
                            @elseif($buy_orderxo->name=='HIGHTLOW')
                                @php($display_text = 'ສູງຕໍ່າ') 
                            @elseif($buy_orderxo->name=='KICKCOOL')
                                @php($display_text = 'ຄີກຄູ່') 
                            @else
                            ...
                            @php($display_text_2 = '')
                            @if($buy_orderxo->type=='UP')
                                @php($display_text_2 = 'ບົນ') 
                            @elseif($buy_orderxo->type=='DOWN')
                                @php($display_text_2 = 'ລ່າງ') 
                            @else
                            
                            @endif
                            ອໍເດີ້ : #HK{{ $buy_orderxo->id }}VIP |
                            ວັນທີ : {{ date('d-m-Y',strtotime($buy_orderxo->created_at))}} |
                            ເວລາ : {{ date('H:i:s',strtotime($buy_orderxo->created_at))}} |
                            ແທງ : {{ $display_text }} -- {{ $display_text_2 }}
                        </div>
                        <table class="  table table-bordered custom-table mt-2" width="50%"  style="border-radius: 10px !important;border: solid 1px black;">
                            <thead> 
